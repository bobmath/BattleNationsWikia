...
sub building_summary {
   ...
   foreach my $id ($build->mission_reqs()) {
      ...
      push @unlock, $mis->wikilink() unless $mis->is_promo();
   }
   push @unlock, 'Unique' if $build->unique();
   print_line($F, 'unlocked', join(', ', @unlock)) if @unlock;
   print_line($F, 'demandcat', $build->demand_cat());
...
sub quest_goods {
   ...
   foreach my $job (@jobs) {
      ...
      my $g = 'good' . ++$n;
      my $mis;
      foreach my $id ($job->missions()) {
         $mis = BN::Mission->get($id) and last;
      }
      print_line($F, $g, $mis ? $mis->wikilink($job->name()) : $job->name());
      print_line($F, $g.'image', BN::Out->icon($job->icon(), 'x40px'));
      if (my $cost = $job->cost()) {
         print_line($F, $g.'time', BN->format_time($cost->{time}));
         print_line($F, $g.'cost',
